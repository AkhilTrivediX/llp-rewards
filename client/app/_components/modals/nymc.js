...
export default function NYMCModal(){
    ...
    const handleLogin = useCallback(async (e)=>{
        e.preventDefault();
        const token = await login(username, password, "nymc");
        if(token){
            localStorage.setItem("token", token);
            localStorage.setItem("brand", "nymc");
            window.location.href = "nymc/coupons";
        }
        else setShowError(true);

    },[username, password])
    return(
        <motion.main initial={{y: "100%"}} animate={{y: "0%"}} exit={{y: "-200%"}} transition={{bounce: 0}}  className="-translate-x-1/2 -translate-y-1/2 z-10 bg-nymc w-[515px] h-[349px] py-6  px-16 flex flex-col items-center justify-center gap-16 absolute top-1/2 left-1/2  rounded-lg">
            <div className="overflow-hidden relative  py-6 px-16 flex flex-col items-center justify-center gap-16">
                <motion.div initial={{y: "-200%"}} animate={{y: "0%"}} exit={{y: "-200%"}} transition={{delay: 0.1, bounce: 0}} ><Image src={"/logos/nymc.svg"} width={309} height={102} alt="NYMC Logo"/></motion.div>
            ...
            <AnimatePresence>
                {showError && <motion.div initial={{y: "50px", opacity: 0}} animate={{y: "0px", opacity: 1}} exit={{y: "50px", opacity: 0}} transition={{delay: 0, bounce: 0, duration: 0.2}} className="absolute top-[105%] left-1/2 -translate-x-1/2 z-20 bg-nymc text-white p-2 flex items-center gap-2 w-[max-content] font-chivo">
