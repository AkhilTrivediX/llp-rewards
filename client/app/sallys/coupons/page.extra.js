...
export default function Page(){
    ...
    useEffect(()=>{
        ...
        if(!token || !brand || brand !== "sallys") router.push("/");
    ...
    return(
        <main className={`flex flex-col relative overflow-x-hidden bg-white ${freeScroll?"":"h-screen overflow-hidden"}`}>
            <div className="flex items-center justify-between relative overflow-hidden">
                ...
                <div className="flex mr-4 gap-2">
                    ...
                    <div className="flex flex-col font-playfair h-full justify-between items-start">
                        <div className="text-xl font-bold leading-none">{username}</div>
            ...
            <div className="relative w-full h-[450px] z-10">
                ...
                <motion.div initial={{opacity: 0, y: "100px"}} animate={{opacity: 1, y: "0px"}} transition={{delay: globalDelay, bounce: 0}} className="w-full h-full flex flex-col items-center justify-center">
            ...
            <div className="flex flex-col p-4 gap-4">
                <motion.div initial={{opacity: 0, x: "100px"}} animate={{opacity: 1, x:"0px"}} transition={{delay: globalDelay, bounce: 0}} className="font-chivo text-4xl font-bold px-8 pb-4 text-black">You have {coupons.length || "no"} {coupons.length === 1 ? "coupon" : "coupons"} available{coupons.length?":":"."}</motion.div>
                <div className="grid md:grid-cols-2 gap-4">
                    {coupons.map((coupon, i)=>(<Coupon key={i} delay={globalDelay+i*0.1} description={coupon.description} expiry={coupon.expiryTime} style={i%3==2} highlighted={i%3==2}/>))}
                </div>
            </div>
            <AnimatePresence mode="popLayout" onExitComplete={()=>{setFreeScroll(true)}}>
                {!username && <motion.div initial={{y: "0%"}} exit={{y: "100%"}} transition={{delay: 0.4, bounce: 0}} className="pointer-events-none w-full h-screen bg-sallys absolute top-0 left-0 z-[10] flex items-center justify-center">
                    <motion.div initial={{y: "0px"}} animate={{y: "500px"}} transition={{delay: 0.3}}>
                        <SallysLogo className="w-[300px] h-[200px] ml-4 fill-white"/>
                    </motion.div>
                </motion.div>}
            </AnimatePresence>
        </main>
